...
// 🔹 FMI API Endpoint & Parameters (Modified for Next 1 Hour)
...
const params = {
    ...
    starttime: new Date(new Date().getTime() - 2 * 60 * 60 * 1000).toISOString(), // Last 2 hours
    endtime: new Date().toISOString(), // Current time
    timestep: "10", 
};


// 🔹 Fetch and Process Aurora Data
async function getAuroraForecast(targetLat, targetLon) {
    try {
        console.log(`🌐 Fetching aurora forecast for upcoming hour at location: ${targetLat}, ${targetLon}`);
        ...
        console.log("📡 XML Data Retrieved (Preview):", xmlData.substring(0,2000));

        // 🔹 Find the closest magnetometer station and extract its geomagnetic data
        const geomagneticValues = filterClosestLocationData(xmlData, targetLat, targetLon);

        if (!geomagneticValues.length) {
            console.log("⚠️ No valid geomagnetic data found for the upcoming hour.");
            return { auroracanbeseen: "0%", letusseeaurora: false, recommendation: "No aurora visibility expected." };
        }

        // 🔹 Calculate aurora probability
        const auroraProbability = calculateAuroraProbability(geomagneticValues);

        // 🔹 Determine forecast & recommendations
        const auroraForecast = auroraProbability >= 60 ? "High" : 
                               auroraProbability >= 40 ? "Moderate" : 
                               auroraProbability >= 20 ? "Low" : "Very Low";

        const recommendation = auroraProbability >= 60 ? "Great chance! Go outside and check." :
                               auroraProbability >= 40 ? "Possibility exists, check the sky!" :
                               "Chances are very low. Try again another night.";
...
function extractGeomagneticValues(xmlData, targetLat, targetLon) {
    const parameterRegex = /<BsWfs:BsWfsElement[^>]*>[\s\S]*?<gml:pos>(.*?)<\/gml:pos>[\s\S]*?<BsWfs:Time>(.*?)<\/BsWfs:Time>[\s\S]*?<BsWfs:ParameterValue>(-?\d+\.\d+)<\/BsWfs:ParameterValue>/g;
    ...
    console.log("📊 Extracted Latest Geomagnetic Values (nT) for Upcoming Hour:", filteredValues);
...
//getAuroraForecast(69.75852, 26.99898); // Location for upcoming hour keva


getAuroraForecast(69.04468714845103, 20.802913138704458); // Location for upcoming hour Kilpisjärvi